<?php
if(trim(DecData($_GET["poc"], 1, $objBF)) == 'auto' && trim(DecData($_GET["i"], 1, $objBF)) != '' && trim(DecData($_GET["rb"], 1, $objBF)) == $LoginUserInfo["user_id"]){
			...
			$trans_amount = $AdvanceSalaryDetail["salary_amount"];
			$objQayaduser->resetProperty();
			$GetRequestedUserName = $objQayaduser->GetUserFullName($PaymentRequest["user_id"]);
			$trans_title = 'Advance Salary issue to '.$GetRequestedUserName;
			$trans_note = 'Advance Salary issue to '.$GetRequestedUserName . ' on '.date("d/m/Y");
			}
?>
			<input type="hidden" name="pri" value="<?php echo trim(DecData($_GET["i"], 1, $objBF));?>">
            <input type="hidden" name="asi" value="<?php echo $AdvanceSalaryDetail["advance_salary_id"];?>">
            <input type="hidden" name="trans_type" id="trans_type" value="3">
            <input type="hidden" name="aplic_mode" value="12">
            <div class="row">
              <label class="col-sm-2 label-on-left">Select Head:</label>
              <div class="col-sm-9">
                <div class="form-group label-floating">
                  <select class="selectpicker" data-style="select-with-transition" name="head_id" id="general_head_id" title="List of Salary Head" required tabindex="2">
                    <?php
					$objQayadaccount->resetProperty();
					$objQayadaccount->setProperty("isActive", 1);
					if($PaymentRequest["user_id"] !=''){
					$objQayadaccount->setProperty("entity_id", $PaymentRequest["user_id"]);	
					}
                    ...
                    ?>
                    <option value="<?php echo EncData($AccountHeadList["head_id"].'-'.$AccountHeadList["entity_id"], 1, $objBF);?>"> <?php echo $AccountHeadList["head_code"] . ' - ' . $AccountHeadList["head_title"] . ' ';?> </option>
                    <?php } ?>
                  </select>
                </div>
              </div>
              ...
              <div class="row">
                ...
                <div class="col-sm-9">
                  <div class="form-group label-floating<?php echo is_form_error($vResult,'trans_amount');?>">
                    ...
                    <input class="form-control" type="text" name="trans_amount" onkeyup="word.innerHTML=convertNumberToWords(this.value)" onfocus="word.innerHTML=convertNumberToWords(this.value)" id="trans_amount" required value="<?php echo $trans_amount;?>" tabindex="2" /><label><small id="word"></small></label>
                  </div>
                </div>
              </div>
              <div class="row">
