	...
	for($li=0;$li<=count($leads_id);$li++){
		if($leads_id[$li] != ''){
			...
			$objQayaduser->actLeads('U');
				...
				$GetLastUserID = $objQayaduser->dbFetchArray(1);
				
					$objQayaduser->resetProperty();
					$objQayaduser->setProperty("assign_lead_id", $GetLastUserID["assign_lead_id"]);
					$objQayaduser->setProperty("assign_lead_status", 2);
			...
			$link = Route::_('show=assignedleads');
			redirect($link);
}
?>