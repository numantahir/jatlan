...
if($_SERVER['REQUEST_METHOD'] == "POST"){
	...
	$trani					= trim($_POST["trani"]);
	$entery_date			= date('Y-m-d H:i:s');
	
	$objValidate->setCheckField("customer_name", 'Supplier Name' . _IS_REQUIRED_FLD, "S");
	//$objValidate->setCheckField("location_id", 'Location' . _IS_REQUIRED_FLD, "S");
	//$objValidate->setCheckField("customer_category", 'Category' . _IS_REQUIRED_FLD, "S");
	...
	if(!$vResult){
				...
				$objSSSjatlan->setProperty("customer_category", $customer_category);
				$objSSSjatlan->setProperty("customer_phone", $customer_phone);
				$objSSSjatlan->setProperty("customer_mobile", $customer_mobile);
				$objSSSjatlan->setProperty("customer_email", $customer_email);
				...
				if($objSSSjatlan->actCustomers($mode)){
						...
						if($mode == 'I'){
							$head_code = 'JTS-'.$customer_id;
							$Head_Title = $customer_business_name;
							$head_description = $customer_business_name. ' Supplier Head';
							...
							$objQayadaccount->setProperty("head_type_id", 6); // Supplier Type
							...
							$head_description = $GetCustomerHead["head_title"]. ' Supplier Head';
						}
						...
						if($TranMode == 1){
							...
							$objQayadaccount->resetProperty();
							$objQayadaccount->setProperty("transaction_id", $trani);
							$objQayadaccount->setProperty("trans_mode", $trans_mode);
							$objQayadaccount->setProperty("trans_amount", $opening_balance);
							$objQayadaccount->setProperty("trans_date", $OpeningDate);
				...
				$objQayaduser->setProperty("log_type", 6);
				$objQayaduser->setProperty("entity_id", $location_id);
				$objQayaduser->setProperty("entery_date", date('Y-m-d H:i:s'));
				if($mode == 'I'){
				$objQayaduser->setProperty("activity_detail", $objQayaduser->fullname." Add New Supplier -> (". $customer_name .")");
				} else {
				$objQayaduser->setProperty("activity_detail", $objQayaduser->fullname." Modify Supplier Info of -> (". $customer_name .")");
				}
				$objQayaduser->actUserLog("I");
				
						$objCommon->setMessage('Supplier information saved successfully.', 'Info');
						$link = Route::_('show=suppliers');
						redirect($link);
				}
