...
die();*/
	...
	$supply_order_id			= trim($_POST["supply_order_id"]);
	...
	if(!$vResult){
			...
			//$transaction_id = ($_POST['mode'] == "U") ? trim($objBF->decrypt($_POST['transaction_id'], 1, ENCRYPTION_KEY)) : $objQayadaccount->genCode("rs_tbl_account_transaction", "transaction_id");
				...
				$objSSSjatlan->setProperty("order_request_id", trim($objBF->decrypt($supply_order_id, 1, ENCRYPTION_KEY)));
			...
			$objQayadaccount->setProperty("location_id", trim($objBF->decrypt($supply_order_id, 1, ENCRYPTION_KEY)));
