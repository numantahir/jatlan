...
if($_SERVER['REQUEST_METHOD'] == "POST"){
	...
	$user_marital_status= trim($_POST['user_marital_status']);
	...
	//$objValidate->setCheckField("device_uid", _REG_DEVICE . _IS_REQUIRED_FLD, "S");
	if($mode == 'I'){
	...
	if(!$vResult){
		...
		$CountUserMobile = $objQayaduser->totalRecords();
			...
			if($CountUserMobile < 1){
				...
				$user_id = ($_POST['mode'] == "U") ? trim($objBF->decrypt($_POST['user_id'], 1, ENCRYPTION_KEY)) : $objQayaduser->genCode("rs_tbl_users", "user_id");
				$objQayaduser->resetProperty();
				...
				$objQayaduser->setProperty("device_uid", $device_uid);
				$objQayaduser->setProperty("user_mobile", $user_mobile);
				if($mode == 'U' && $user_pass!=''){
				...
				$objQayaduser->setProperty("blood_group", $blood_group);
				$objQayaduser->setProperty("user_gender", $user_gender);
				$objQayaduser->setProperty("user_dob", dateFormate_10($user_dob));
				$objQayaduser->setProperty("user_marital_status", $user_marital_status);
				$objQayaduser->setProperty("vehicle_id", $vehicle_id);
				if($objQayaduser->actUser_HR($mode)){
					
						
						if($mode == 'I'){
							$head_code = 'JTE-'.$user_id;
							$head_title = $user_fname.' '.$user_lname;
							$head_description = $user_fname.' '.$user_lname. ' Employee Head';
							...
							$objQayadaccount->setProperty("entity_id", $user_id);
							$objQayadaccount->setProperty("entery_date", $reg_date);
				...
				if($mode == 'I'){
				$objQayaduser->setProperty("activity_detail", "Create New User by ".$LoginUserInfo["fullname"]." -> (".$user_fname ." ".$user_lname.")");
				} else {
				$objQayaduser->setProperty("activity_detail", $LoginUserInfo["fullname"]." Edit user info -> (".$user_fname ." ".$user_lname.")");
				}
				...
				if(is_uploaded_file($_FILES['cnic_front_side']['tmp_name'])){
				$objQayaduser->resetProperty();
				$cnic_front_side_name = $objQayaduser->getImagename($_FILES['cnic_front_side']['type'], $user_id);
				if(move_uploaded_file($_FILES['cnic_front_side']['tmp_name'], USER_DOCUMENT_PATH . $cnic_front_side_name)){
				$cnic_front_sidefolder = USER_DOCUMENT_PATH . $cnic_front_side_name;
				$cnic_front_sideThumbFolder = USER_DOCUMENT_THUMB_PATH;
				$cnic_front_sideThumbPath = USER_DOCUMENT_THUMB_PATH . $cnic_front_side_name;
				copy($cnic_front_sidefolder, $cnic_front_sideThumbPath);
				$ResizeSig = new Thumbnail($cnic_front_sideThumbPath, 165, 0, 100);
				$ResizeSig->save($cnic_front_sideThumbPath);
				}
				$objQayaduser->resetProperty();
				$objQayaduser->setProperty("cnic_front_side", $cnic_front_side_name);
				$objQayaduser->setProperty("user_mobile", $user_mobile);
				$objQayaduser->actUser("U");
				}
				
				if(is_uploaded_file($_FILES['cnic_back_side']['tmp_name'])){
				$objQayaduser->resetProperty();
				$cnic_back_side_name = $objQayaduser->getImagename($_FILES['cnic_back_side']['type'], $user_id);
				if(move_uploaded_file($_FILES['cnic_back_side']['tmp_name'], USER_DOCUMENT_PATH . $cnic_back_side_name)){
				$cnic_back_sidefolder = USER_DOCUMENT_PATH . $cnic_back_side_name;
				$cnic_back_sideThumbFolder = USER_DOCUMENT_THUMB_PATH;
				$cnic_back_sideThumbPath = USER_DOCUMENT_THUMB_PATH . $cnic_back_side_name;
				copy($cnic_back_sidefolder, $cnic_back_sideThumbPath);
				$ResizeSig = new Thumbnail($cnic_back_sideThumbPath, 165, 0, 100);
				$ResizeSig->save($cnic_back_sideThumbPath);
				}
				$objQayaduser->resetProperty();
				$objQayaduser->setProperty("cnic_back_side", $cnic_back_side_name);
				$objQayaduser->setProperty("user_mobile", $user_mobile);
				$objQayaduser->actUser("U");
				}
				
				if(is_uploaded_file($_FILES['user_cv']['tmp_name'])){
				$objQayaduser->resetProperty();
				$user_cv_name = $objQayaduser->getImagename($_FILES['user_cv']['type'], $user_id);
				if(move_uploaded_file($_FILES['user_cv']['tmp_name'], USER_DOCUMENT_PATH . $user_cv_name)){}
				$objQayaduser->resetProperty();
				$objQayaduser->setProperty("user_cv", $user_cv_name);
				$objQayaduser->setProperty("user_mobile", $user_mobile);
				$objQayaduser->actUser("U");
				}
