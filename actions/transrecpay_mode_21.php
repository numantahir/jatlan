...
if($_SERVER['REQUEST_METHOD'] == "POST"){
	...
	if(!$vResult){
			...
			$objQayadaccount->setProperty("aplic_mode", 11);
			...
			$objQayadaccount->setProperty("location_id", 1);
			...
			if($objQayadaccount->actAccountTransaction('I')){
				...
				********************************************************************/
				...
				$objQayadapplication->setProperty("aplic_id", trim($objBF->decrypt($entery_id, 1, ENCRYPTION_KEY)));
				$objQayadapplication->setProperty("discount_apply", $discount_apply);
				$objQayadapplication->setProperty("discount_option", $discount_option);
				$objQayadapplication->setProperty("discount_on", $discount_on);
				$objQayadapplication->setProperty("discount_value", $discount_value);
				$objQayadapplication->setProperty("instalment_option", $instalment_option);
				$objQayadapplication->setProperty("no_of_instalment", $no_of_instalment);
				$objQayadapplication->setProperty("instalment_due_on", $instalment_due_on);
				$objQayadapplication->actPaymentOverview('U');*/
