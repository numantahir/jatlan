...
if($_SERVER['REQUEST_METHOD'] == "POST" && $_POST['mode'] == "S"){
	...
	if(!$vResult){
                    ...
                    $GetTransactionDetail = $objQayadaccount->dbFetchArray(1);
						...
						$link = Route::_('show=wrongtransactionform&rc='.EncData('get', 2, $objBF).'&tn='.EncData($wrong_tran_no, 2, $objBF).'&ti='.EncData($GetTransactionDetail["transaction_id"], 2, $objBF));
						redirect($link);
						
					} else {
						$link = Route::_('show=wrongtransactionform&e='.EncData('wrong', 2, $objBF).'&tn='.EncData($wrong_tran_no, 2, $objBF));
						redirect($link);	
					}
	...
	if(!$vResult){
				...
				if($objQayadaccount->actWrongTransactions('I')){
						...
						if($Credit_Tran_id != '' && $Credit_Tran_id !=0){
							...
							$objQayadaccount->setProperty("isActive", 5);
				...
				$objQayaduser->setProperty("log_type", 24);
				...
				$objQayaduser->actUserLog("I");
						...
						$link = Route::_('show=wrongtransaction');
						redirect($link);
				}
