	...
	if(!$vResult){
		...
		$CountUserMobile = $objQayaduser->totalRecords();
			...
			if($CountUserMobile < 1){
				...
				if($mode == 'I'){
				$objQayaduser->setProperty("activity_detail", "Create New Agent by ".$objQayaduser->fullname." -> (".$user_fname ." ".$user_lname.")");
				} else {
				$objQayaduser->setProperty("activity_detail", $objQayaduser->fullname." Edit agent info -> (".$user_fname ." ".$user_lname.")");
				}
