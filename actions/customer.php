...
if($_SERVER['REQUEST_METHOD'] == "POST"){
	...
	if(!$vResult){
				...
				if($objSSSjatlan->actCustomers($mode)){
						...
						if($mode == 'I'){
							$head_code = 'JTC-'.$customer_id;
							$Head_Title = $customer_name;
							$head_description = $customer_name. ' Customer Head';
							...
							$head_description = $GetCustomerHead["head_title"]. ' Customer Head';
						}
				...
				$objQayaduser->setProperty("log_type", 5);
				$objQayaduser->setProperty("entity_id", $location_id);
				$objQayaduser->setProperty("entery_date", date('Y-m-d H:i:s'));
				if($mode == 'I'){
				$objQayaduser->setProperty("activity_detail", $objQayaduser->fullname." Add New Customer -> (". $customer_name .")");
				} else {
				$objQayaduser->setProperty("activity_detail", $objQayaduser->fullname." Modify Customer Info of -> (". $customer_name .")");
				}
				$objQayaduser->actUserLog("I");
				
						$objCommon->setMessage('Customer information saved successfully.', 'Info');
						$link = Route::_('show=customers');
						redirect($link);
				}
