      ...
      <div class="col-md-12">
        <div class="card">
          ...
          <div class="card-content">
            <?php if($_GET["i"]==''){?>
            <h3 class="card-title CardWidth">Employees Management</h3>
            <div class="toolbar add-btn text-right">
              <?php //if($objCheckLogin->user_type == 9 or $objCheckLogin->user_type == 16){ ?>
              <a href="<?php echo Route::_('show=employeform');?>" class="btn btn-primary">Add New</a>
              <?php //} ?>
            </div>
            <hr>
            <div class="material-datatables">
              <table id="datatables" class="table table-striped table-no-bordered table-hover employees-management" cellspacing="0" width="100%" style="width:100%">
                <thead>
                  <tr>
                    <th>Name</th>
                    <th>Device ID</th>
                    ...
                    <th>Shift</th>
                    <th>Type</th>
                    <?php if($objCheckLogin->user_type == 9 or $objCheckLogin->user_type == 16 or $objCheckLogin->user_type == 1){ ?>
                    <th>Action</th>
                    <?php } ?>
                  </tr>
                </thead>
				...
				//1=>Admin, 2=>Front Desk, 3=>Finance, 4=>Agent, 5=>Sales, 6=>SMS Account, 7=>Director, 8=>Creative-Director, 9=>HR, 10=>Employee
					...
					$objQayadShiftName = new Qayaduser;
					...
					while($ListOfUsers = $objQayaduser->dbFetchArray(1)){
						...
						$objQayadShift->setProperty("user_id", $ListOfUsers["user_id"]);
						$objQayadShift->lstUserShifts();
						$UserShiftDetail = $objQayadShift->dbFetchArray(1);
						
				?>
                  <tr>
                    <td><a href="<?php echo Route::_('show=employees&i='.EncData($ListOfUsers["user_id"], 2, $objBF));?>"><?php echo $ListOfUsers["fullname"];?></a></td>
                    <td><?php echo $ListOfUsers["device_uid"];?></td>
                    <td><?php echo $ListOfUsers["user_designation"];?></td>
                    <td><?php echo $ListOfUsers["user_mobile"];?></td>
                    <td><?php echo $objQayadShiftName->GetShiftName($UserShiftDetail["shift_id"]);?></td>
                    <td><?php echo UserType($ListOfUsers["user_type_id"]);?></td>
                    <?php if($objCheckLogin->user_type == 9 or $objCheckLogin->user_type == 16 or $objCheckLogin->user_type == 1){ ?>
                    <td class="td-actions"><a href="<?php echo Route::_('show=employeopt&i='.EncData($ListOfUsers["user_id"], 2, $objBF));?>" type="button" rel="tooltip" class="btn btn-success btn-simple form-edit"> <i class="material-icons">storage</i> </a></td>
                    <?php } ?>
                  </tr>
                  <?php } ?>
                </tbody>
